...
//Code Here
let myStrongest = monstersInYourPocket.filter(monster => monster.CP > 200 ? monster : false)
...
let bobsTotal = purchases.filter( val => val['owner'] === 'Bob').reduce((acc, elem) => {
  return acc += elem['price']
}, 0)

// let bobsTotal = purchases.filter(val => val[owner] === 'Bob' ? val : false).reduce((acc, elem) => {
//   return acc += elem['price']}, 0)
